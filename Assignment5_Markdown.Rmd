...
enroll_graph <- ggplot(grad, aes(x = Year, y = Total_Enrollment))+
  #geom_point(aes(x = Year, y = Total_Males))+
  geom_smooth(aes(x = Year, y = Total_Males), method = lm, alpha = 0.15, se = F)+
  geom_smooth(aes(x = Year, y = Total_Females), method = lm, alpha = 0.15, se = F)
